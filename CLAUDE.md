...
| Frontend: `Provider`, landing + mapa + aprende + chatbot, rutas `/`, `/mapa`, `/aprende`, `/chatbot` (actualizado 2026-07-23 según Figma) | **`Provider` ✅ cerrado** (2026-07-23): se migró y se eliminó `flutter_bloc`. **Rutas ✅ ya coinciden** con el objetivo. Sigue pendiente: la página de detalle de sector (pestañas Resumen/Historia/Sensores, alcanzada desde el mapa) todavía no existe — ver EPI-04 en `02-Backlog.md` |
...
- **Navegación:** `go_router`. Las rutas están centralizadas como constantes (`AppRoutes`) en `lib/core/router/app_router.dart`, pero el `GoRouter` real (`appRouter`) hoy vive en `lib/app.dart`, junto a una clase `EcoBytesApp` que no se usa. Nota: hay dos archivos con lógica de app raíz — `lib/main.dart` (`MyApp`, usado como entrypoint real, con `MultiProvider`) y `lib/app.dart` (`EcoBytesApp`, sin providers, no se usa como widget raíz) — antes de tocar el arranque de la app, confirma cuál `MaterialApp.router` está realmente en uso. Pendiente de resolver: mover `appRouter` a `core/router/app_router.dart` y borrar `EcoBytesApp`/`lib/app.dart` (`06-Plan-de-Accion.md` §3 paso 1).
