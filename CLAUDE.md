...
**Nota (2026-07-23, actualizada 2026-07-25):** landing y chatbot dejaron de ser deuda pendiente — están respaldados por el Figma del equipo. `T-00.4` del backlog quedó obsoleta. **La pausa del chatbot terminó el 2026-07-25:** está implementado de punta a punta — `POST /chatbot` (6º endpoint, OpenAI) y la pantalla Flutter conectada vía `ChatbotProvider`.
...
- **`services/chatbot_context.py`** — construye el snapshot de datos reales que se le inyecta al LLM (comunas sin `geometry`, agregados de ciudad, umbrales OMS y un subconjunto de `educacion.json` — filtra `ui` y `niveles_calidad`, que son copy de la pantalla Aprende). Cacheado 60 s. Si ClickHouse falla, **degrada en vez de romper**: marca `datos_sensores_disponibles: false` y no cachea ese estado.
...
- **Features (`lib/features/`)**, organizadas por dominio con subcarpetas `data/`, `domain/`, `presentation/` (bloc + pages + widgets) donde aplica:
  ...
  - `chatbot/` — **conectado al backend real** (2026-07-25). `ChatbotProvider` habla con `POST /chatbot` vía `ApiClient.postChatbot()`; se eliminó la conversación de ejemplo hardcodeada. Dos cosas que no son obvias: el **historial vive en el cliente** y viaja completo en cada petición (el backend no guarda estado, por eso el provider se registra en `main.dart` y no por ruta), y el badge de estado tiene **tres** valores, no dos — `desconocida` hasta el primer mensaje, porque hasta entonces no hay forma de saber si el servidor tiene `OPENAI_API_KEY` y decir "En línea" sería inventar.
  - `learn/` — contenido educativo sobre PM2.5/CO2, **conectado a `GET /education`** (2026-07-25) vía `EducationProvider`. Ya no tiene contenido propio: todo sale de `Backend/data/educacion.json`, el mismo archivo que alimenta al chatbot. Dos cosas no obvias: sigue funcionando **sin backend** gracias a `assets/educacion.json` (copia literal del archivo del backend, con aviso en pantalla cuando se usa), y los umbrales y colores de los cuatro estados **no** se leen del JSON sino de `EstadoSector`, el enum del mapa.
