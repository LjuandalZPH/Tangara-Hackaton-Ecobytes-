...
| Módulo Riesgo (`risk/`) | Selector de barrio → perfil histórico (`risk_provider.dart`, `risk_profile_card.dart`) | **Capa de datos ✅ cerrada** (commit `a44940b`, 2026-07-23): existen `domain/models/riesgo.dart` y `presentation/providers/risk_provider.dart` (fetch bajo demanda contra `GET /risk/{sector}`, caché en memoria por sector, ya registrado en el `MultiProvider` de `main.dart`). **Sigue sin existir la UI**: no hay `risk_page.dart`, selector de comuna, tarjeta de perfil, ni ruta `/riesgo` en `AppRoutes` — el provider está vivo en el árbol de widgets pero ninguna pantalla lo consume todavía |
